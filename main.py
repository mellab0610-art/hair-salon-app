...
with st.sidebar:
    ...
    api_key = st.text_input(
        ...
        type="password",
        placeholder="sk-ant-...",
        help="Anthropic Consoleで発行したAPIキーを入力してください。",
    )
    st.markdown("---")
...
def build_prompt(store_name, concern, region, booked_menu, actual_menu, point, reaction, other):
    ...
    prompt = f"""あなたは福岡の髪質改善専門サロンのスタイリストです。
...
if generate_btn:
    errors = []
    if not api_key:
        errors.append("サイドバーにAnthropicのAPIキーを入力してください。")
    if not before_file:
    ...
    with st.spinner("Claude APIで投稿文を生成しています..."):
        try:
            prompt = build_prompt(
            ...
            st.error(f"文章生成中にエラーが発生しました: {ex}")
            st.stop()
